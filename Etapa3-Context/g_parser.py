...
def p_program(p):
    ...
    '''
    if (len(p)==4):
        p[0] = SyntaxLeaf('Block', None, [p[2]],p.lineno(1),find_column2(p.lexer.lexdata,p,1))
    elif (len(p)==6):
        p[0] = SyntaxLeaf('Block', None, [p[3],p[4]],p.lineno(1),find_column2(p.lexer.lexdata,p,1))
    else:
        p[0] = SyntaxLeaf('Block', None, [p[3]],p.lineno(1),find_column2(p.lexer.lexdata,p,1))
...
def p_declaration(p):
    ...
    '''
    if (len(p)==4):
        p[0] = SyntaxLeaf('Declare', p[3], [p[1],p[3]], p.lineno(2),find_column2(p.lexer.lexdata,p,2))
    else:
        p[0] = SyntaxLeaf('Declare', p[3], [p[1],p[3],p[4],p[5]],p.lineno(2),find_column2(p.lexer.lexdata,p,2))      
...
def p_array(p):
    ...
    '''
    p[0] = SyntaxLeaf('Array', None, [p[3],p[5]],p.lineno(4), find_column2(p.lexer.lexdata,p,4))
...
def p_terminal(p):
    ...
    '''
    if (len(p) == 4):
        p[0] = SyntaxLeaf('Terminal', None, [p[2]],p.lineno(1), find_column2(p.lexer.lexdata,p,1))
        ...
        p[0] = SyntaxLeaf('Terminal', p[1],None,p.lineno(1), find_column2(p.lexer.lexdata,p,1))
        if ((str(p[1])).isdigit()):
            p[0].c_type = "int"
            p[0].c_lexeme = p[1]
        elif (p[1] == "true" \
...
def p_content(p):
    ...
    '''
    if (len(p) == 2):
        p[0] = SyntaxLeaf('Content', None, [p[1]],p.lineno(1), find_column2(p.lexer.lexdata,p,1))
    else:
        p[0] = SyntaxLeaf('Content', None, [p[1], p[2], p[3]],p.lineno(2), find_column2(p.lexer.lexdata,p,2))
...
def p_instruction(p):
    ''' Instruction :   Conditional
                    ...
                    |   Output
    '''
    p[0] = SyntaxLeaf('Instruction', None, [p[1]], p.lineno(1), find_column2(p.lexer.lexdata,p,1))
...
def p_conditional(p):
    ...
    '''
    if (len(p)==6):
        p[0] = SyntaxLeaf('Conditional', None, [p[2],p[4]], p.lineno(1), find_column2(p.lexer.lexdata,p,1))
    else:
        p[0] = SyntaxLeaf('Conditional', None, [p[2],p[4],p[5]],p.lineno(1), find_column2(p.lexer.lexdata,p,1))
...
def p_guard(p):
    ...
    '''
    if (len(p)==5):
        p[0] = SyntaxLeaf('Guard', None, [p[2],p[4]],p.lineno(1), find_column2(p.lexer.lexdata,p,1))
    else:
        p[0] = SyntaxLeaf('Guard', None, [p[2],p[4],p[5]],p.lineno(1), find_column2(p.lexer.lexdata,p,1))
...
def p_asign(p):
    ...
    '''
    p[0] = SyntaxLeaf('Asign', p[1], [p[3]], p.lineno(2), find_column2(p.lexer.lexdata,p,2))
...
def p_asiggnation(p):
    ...
    '''
    if(len(p) == 4):
        p[0] = SyntaxLeaf('Assignation', None, [p[1], p[3]], p.lineno(2), find_column2(p.lexer.lexdata,p,2))
    else:
        p[0] = SyntaxLeaf('Assignation', None, [p[1]],p.lineno(1), find_column2(p.lexer.lexdata,p,1))
...
def p_input(p):
    ''' Input   :   TkRead TkId
    '''
    p[0] = SyntaxLeaf('Input', p[1], [p[2]], p.lineno(1), find_column2(p.lexer.lexdata,p,1))
...
def p_output(p):
    ...
    '''
    if(len(p)==4):
        p[0] = SyntaxLeaf('Output', p[1], [p[2],p[3]],p.lineno(1), find_column2(p.lexer.lexdata,p,1))
    else:
        p[0] = SyntaxLeaf('Output', p[1], [p[2]],p.lineno(1), find_column2(p.lexer.lexdata,p,1))
...
def p_concatexp(p):
    ...
    '''
    if(len(p)==4):
        p[0] = SyntaxLeaf('ConcatExpression', None, [p[2],p[3]],p.lineno(1), find_column2(p.lexer.lexdata,p,1))
    else:
        p[0] = SyntaxLeaf('ConcatExpression', None, [p[2]],p.lineno(1), find_column2(p.lexer.lexdata,p,1))
...
def p_doloop(p):
    ...
    '''
    if(len(p)== 6):
        p[0] = SyntaxLeaf('Doloop', None, [p[2], p[4]],p.lineno(1), find_column2(p.lexer.lexdata,p,1))
    else:
        p[0] = SyntaxLeaf('Doloop', None, [p[2], p[4], p[5]],p.lineno(1), find_column2(p.lexer.lexdata,p,1))
...
def p_forloop(p):
    ...
    ''' 
    p[0] = SyntaxLeaf('Forloop', p[2], [p[4],p[6],p[8]],p.lineno(5), find_column2(p.lexer.lexdata,p,5))
...
def p_expression(p):
    ''' Expression  :   AritmeticOperator
                    ...
                    |   ArrayExpression
    '''
    p[0] = SyntaxLeaf('Expression', None, [p[1]], p.lineno(1), find_column2(p.lexer.lexdata,p,1))
...
def p_datatype(p):
    ...
    '''
    if (len(p)==4):
        p[0] = SyntaxLeaf('Datatype', p[1], [p[3]], p.lineno(1),find_column2(p.lexer.lexdata,p,1))
    else:
        p[0] = SyntaxLeaf('Datatype', p[1],None,p.lineno(1),find_column2(p.lexer.lexdata,p,1))    
